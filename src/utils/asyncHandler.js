const asyncHandler = (requestHandler) => {
  // here requestHandler expecting another function that have
  return (req, res, next) => {
    // three paramaters that req, res, next
    ...
    Promise.resolve(requestHandler(req, res, next)).catch((err) => next(err)); // It passes the error to the next middleware function with next(err). This ensures that any errors are handled by Express’s
